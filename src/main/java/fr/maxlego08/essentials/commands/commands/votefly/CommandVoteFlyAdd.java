...
public class CommandVoteFlyAdd extends VCommand {
    public CommandVoteFlyAdd(EssentialsPlugin plugin) {
        super(plugin);
        this.addSubCommand("add");
        this.setPermission(Permission.ESSENTIALS_VOTEFLY_ADD);
        this.setDescription(Message.DESCRIPTION_VOTEFLY_ADD);
    ...
    protected CommandResultType perform(EssentialsPlugin plugin) {
        ...
        fetchUniqueId(userName, uniqueId -> {
            ...
            if (target == null) {
                ...
                storage.upsertVoteFlySeconds(uniqueId, voteFlySeconds);
            } else {
                target.addVoteFlySeconds(seconds);
            }

            message(sender, Message.COMMAND_VOTEFLY_ADD, "%player%", userName, "%time%", TimerBuilder.getStringTime(seconds * 1000));
        });
