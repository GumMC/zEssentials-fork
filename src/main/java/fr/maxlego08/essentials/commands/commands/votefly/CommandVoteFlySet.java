...
public class CommandVoteFlySet extends VCommand {
    public CommandVoteFlySet(EssentialsPlugin plugin) {
        super(plugin);
        this.addSubCommand("set");
        this.setPermission(Permission.ESSENTIALS_VOTEFLY_SET);
        this.setDescription(Message.DESCRIPTION_VOTEFLY_SET);
    ...
    protected CommandResultType perform(EssentialsPlugin plugin) {
        ...
        fetchUniqueId(userName, uniqueId -> {
            ...
            if (target == null) {
                storage.upsertVoteFlySeconds(uniqueId, seconds);
            } else {
                target.setVoteFlySeconds(seconds);
            }

            message(sender, Message.COMMAND_VOTEFLY_SET, "%player%", userName, "%time%", TimerBuilder.getStringTime(seconds * 1000));
        });
